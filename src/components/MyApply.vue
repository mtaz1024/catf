  ...
  <div class="manage-content">
    ...
    <el-table
      ...
      <el-table-column label="发布者"
                       ...
                       width="120">
        <template slot-scope="scope">
          <div style="font-weight: bold" @click="toHomePage(scope.row.posterId)">{{ scope.row.posterName }}</div>
        </template>
      </el-table-column>
      <!-- 发布时间 -->
        ...
        <template slot-scope="scope">
          <div v-if="scope.row.adoptState === 0">
            <el-button
                ...
                type="danger"
                @click="cancel(scope.$index, scope.row)">取消申请</el-button>
          </div>
          <div v-if="scope.row.adoptState !== 0">
            <el-button
                disabled
                ...
                style="background-color: black; border-color: black; color: #efc239;font-weight: bold"
                type="danger"
                @click="cancel(scope.$index, scope.row)">已结束</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    getMyApplyList() {
      ...
      this.$api.adopt.getMyApply({
        ...
        if (res.data.code === 200){
          ...
          that.applyList = res.data.data.list
    ...
    cancel(index, apply) {
      console.log(index, apply);
      this.$api.adopt.cancel({
        adoptId: apply.adoptId
      })
      .then(res => {
        if (res.data.code === 200){
          this.$message({
